...
- (instancetype)initWithSignatureVerificationKeyObject:(SignatureVerificationKey *)signatureVerificationKey {
    self = [self init];
    if (self != NULL) {
...
+ (instancetype)deriveFromSeedWithSeedString:(NSString*)seedString derivationOptionsJson:(NSString*)derivationOptionsJson {
    SignatureVerificationKey key = SigningKey::deriveFromSeed([seedString UTF8String], [derivationOptionsJson UTF8String]).getSignatureVerificationKey();
...
+ (instancetype)fromJsonWithSignatureVerificationKeyAsJson:(NSString*)signatureVerificationKeyAsJson {
    SignatureVerificationKey key = SignatureVerificationKey::fromJson([signatureVerificationKeyAsJson UTF8String]);
...
+ (instancetype)fromSerializedBinaryFrom:(NSData*)serializedBinaryForm {
    ...
    SignatureVerificationKey key = SignatureVerificationKey::fromSerializedBinaryForm(sodiumBuffer);
    return [[DSCSignatureVerificationKey alloc] initWithSignatureVerificationKeyObject: new SignatureVerificationKey(key)];
}

- (NSString*)toJson {
    return [NSString stringWithUTF8String:_signatureVerificationKeyObject->toJson().c_str()];
}

- (NSData*)toSerializedBinaryForm {
    SodiumBuffer sodiumBuffer = _signatureVerificationKeyObject->toSerializedBinaryForm();
    return sodiumBufferToData(sodiumBuffer);
}

- (NSString*)derivationOptionsJson {
    return [NSString stringWithUTF8String:_signatureVerificationKeyObject->derivationOptionsJson.c_str()];
}

- (NSData*)signatureVerificationKeyBytes {
    return unsignedCharVectorToData(_signatureVerificationKeyObject->signatureVerificationKeyBytes);
}

- (BOOL)verifyWithMessage:(NSString *)message signature:(NSData *)signature {
    return _signatureVerificationKeyObject->verify(stringToUnsignedCharArray(message), message.length, dataToUnsignedCharVector(signature));
}
